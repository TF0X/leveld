...
export default function Cravings() {
  ...
  const [form, setForm] = useState({ type: 'food', intensity: 5 })
  ...
  const startTimer = () => {
    setLastCraving({ ...form, resisted: true })
    setPhase('timer')
  }

  const handleResisted = async () => {
    const craving = { ...form, resisted: true }
    logCraving(craving)
    gainXP(25, 'craving')
    addWillpower()
    setPhase('result')
  ...
  const handleGaveIn = async () => {
    ...
    const craving = { ...form, resisted: false }
    logCraving(craving)
    ...
    <div className="space-y-4 pb-6">
      ...
      {phase === 'logging' && (
        <div className="rpg-panel p-6 border border-violet-700 space-y-4">
          ...
          <div>
            <label className="text-xs text-slate-400 block mb-1">Intensity: {form.intensity}/10</label>
            <input
      ...
      {phase === 'result' && lastCraving && (
        <div className={`rpg-panel p-6 text-center border ${lastCraving.resisted ? 'border-green-600' : 'border-red-700'}`}>
          ...
          <div className="font-pixel text-xs mb-2" style={{ color: lastCraving.resisted ? '#10b981' : '#ef4444' }}>
          ...
          {aiMsg && <div className="text-xs text-slate-300 italic leading-relaxed mb-4">{aiMsg}</div>}
          <button className="rpg-btn-secondary" onClick={() => { setPhase('idle'); setAiMsg('') }}>Continue</button>
        </div>
      )}
